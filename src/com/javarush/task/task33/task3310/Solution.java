...
public class Solution {
    ...
    public static void testStrategy(StorageStrategy strategy, long elementsNumber) {
        com.javarush.task.task33.task3310.Helper.printMessage(strategy.getClass().getSimpleName() + ":");

        Set<String> origStrings = new HashSet<>();

        for (int i = 0; i < elementsNumber; ++i) {
            origStrings.add(com.javarush.task.task33.task3310.Helper.generateRandomString());
        }
        ...
        com.javarush.task.task33.task3310.Helper.printMessage("Время получения идентификаторов для " + elementsNumber + " строк: " + time);

        startTimestamp = new Date();
        Set<String> strings = getStrings(shortener, keys);
        endTimestamp = new Date();
        time = endTimestamp.getTime() - startTimestamp.getTime();
        com.javarush.task.task33.task3310.Helper.printMessage("Время получения строк для " + elementsNumber + " идентификаторов: " + time);

        if (origStrings.equals(strings))
            com.javarush.task.task33.task3310.Helper.printMessage("Тест пройден.");
        else
            com.javarush.task.task33.task3310.Helper.printMessage("Тест не пройден.");

        com.javarush.task.task33.task3310.Helper.printMessage("");
    }
